...
const MessagesList = () => {
    ...
    const currentChannelMessages = orderedMessages.filter(message => message.channelId === currentChannelId)
    return (
        <div id="messages-box" className="chat-messages overflow-auto px-5 ">
            {currentChannelMessages.map(message => <Message key={message.id} username={message.username} text={message.body} />)}
        </div>
    )
}
